...
class PetAssetRepository(private val context: Context) {
    ...
    fun installGeneratedAppearance(pngBytes: ByteArray, idea: String): File {
        val previous = preferences.getString(KEY_CUSTOM_FILE, CUSTOM_FILE_NAME)
            ?.takeIf { it.isNotBlank() }
        val nextName = "custom_generated_${System.currentTimeMillis()}.png"
        val target = File(assetDir, nextName)
        val temp = File(assetDir, "$nextName.tmp")
        ...
        val committed = preferences.edit()
            ...
            .putString(KEY_CUSTOM_FILE, nextName)
            .commit()
        check(committed) { "无法写入自定义外观配置" }
        // 删旧文件，避免 ImageDecoder / 系统按路径缓存旧图
        if (!previous.isNullOrBlank() && previous != nextName) {
            File(assetDir, previous).delete()
        }
        File(assetDir, CUSTOM_FILE_NAME).delete()
        File(assetDir, "$CUSTOM_FILE_NAME.tmp").delete()
        return target
    }

    fun clearCustomAppearance() {
        val current = preferences.getString(KEY_CUSTOM_FILE, CUSTOM_FILE_NAME)
        if (!current.isNullOrBlank()) File(assetDir, current).delete()
        File(assetDir, CUSTOM_FILE_NAME).delete()
        preferences.edit()
            .putBoolean(KEY_HAS_CUSTOM, false)
            .remove(KEY_CUSTOM_IDEA)
            .remove(KEY_CUSTOM_FILE)
            .commit()
        restoreBundledDefaults()
    }
    ...
    /** 当前自定义形象文件；无自定义时返回 null。 */
    fun customAppearanceFilePublic(): File? = customAppearanceFile()

    private fun customAppearanceFile(): File? {
        if (!preferences.getBoolean(KEY_HAS_CUSTOM, false)) return null
        val named = preferences.getString(KEY_CUSTOM_FILE, null)
            ?.takeIf { it.isNotBlank() }
            ?.let { File(assetDir, it).takeIf { f -> f.isFile } }
    ...
    companion object {
        ...
        private const val KEY_CUSTOM_FILE = "custom_appearance_file"
