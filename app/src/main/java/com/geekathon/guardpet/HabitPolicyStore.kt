...
object HabitPolicyStore {
    ...
    var agentEnabled: Boolean
        // 默认关闭：需用户在「作息守护」里主动打开，避免误拦系统设置等应用
    ...
    private fun shouldNeverStoreRule(packageName: String): Boolean {
        ...
        if (self != null && p == self) return true
        if (p.contains("sensevoice")) return true
        if ("launcher" in p) return true
        // 系统设置 / 权限 / 管家：绝不能被商店分类误标成娱乐整包禁
        if (p in SYSTEM_NEVER_BLOCK) return true
        if (p.startsWith("com.android.settings") ||
            ...
            p.startsWith("com.miui.securitycenter") ||
            p.startsWith("com.miui.home") ||
            p.startsWith("com.miui.powerkeeper") ||
            p.startsWith("com.xiaomi.misettings")
        ) {
            return true
        }
        return false
    ...
    private val SYSTEM_NEVER_BLOCK = setOf(
        "android",
